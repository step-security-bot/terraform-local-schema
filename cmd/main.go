...
import (
	...
	"github.com/lonegunmanb/terraform-local-schema/v2/schema"
	"golang.org/x/oauth2"
)

func main() {
	refreshFlag := flag.Bool("refresh", false, "Refresh local schema")
...
func checkGitHubTag(schemaVersion string) (bool, error) {
	...
	for {
		tags, resp, err := client.Repositories.ListTags(context.Background(), "lonegunmanb", "terraform-local-schema", options)
		if err != nil {
			return false, err
		}
